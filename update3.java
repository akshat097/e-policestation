			  ...
			  Class.forName(driver).newInstance();
				java.sql.Connection  con = DriverManager.getConnection(url+dbName);
				  ...
				  int k=0;
					...
					while (rs.next()) {
			             if ((name.equals(rs.getString("name")) && uid.equals(rs.getString("user_id")))) {
			            	  st = con.prepareStatement("UPDATE missing_valuable_form1 SET status = '"+status+"' WHERE  user_id = '"+uid+"' and name='"+name+"'");
					...
					if(k>0)
				 {response.sendRedirect("missupdate3.jsp");}
					else
